...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// ============================================================================================================================================================================
// Module Name : fiFFNTT multiplier
...
localparam Qn = 16'h1D001;              // * negative Q
...
wire [(pNTT_WIDTH*2):0]             mont_add_result[7:0];
...
//   128bit input data * 128bit input data = (8*16bit) * (8*16 bit) = (8*32 bit)result.                      //   
//   (T)result = {mul_16_result_c3[3]  , mul_16_result_c2[2] ,  mul_16_result_c1[1] , mul_16_result_c0[0]    //
//             , mul_16_result_b3[3] , mul_16_result_b2[2] , mul_16_result_b1[1] , mul_16_result_b0[0]}      //
//   (W)mul16_array_ntt row2 row3 output = (T*Q01 mod R) * Q                                                 //
...
assign array_in_A1       = (state == INT_MUL )? array_in_ntt1 : {{(pFP_WIDTH - pMANTISSA_WIDTH -1 ){1'b0}} , hidden_a_im      , a_im_r    [(pMANTISSA_WIDTH-1):0]};
assign array_in_A2       = (state == INT_MUL )? array_in_ntt2 : {{(pFP_WIDTH - pMANTISSA_WIDTH -1 ){1'b0}} , hidden_a_im      , a_im_r    [(pMANTISSA_WIDTH-1):0]};
...
CLA33 CLA33_1(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a0[2]}), .B(mul_16_reg[8][(pNTT_WIDTH*2-1):0]),                .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[0]), .result(mont_add_result[0]));
CLA33 CLA33_2(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a1[2]}), .B(mul_16_reg[8][(pNTT_WIDTH*4-1):(pNTT_WIDTH*2)]),   .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[1]), .result(mont_add_result[1]));
CLA33 CLA33_3(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a2[2]}), .B(mul_16_reg[8][(pNTT_WIDTH*6-1):(pNTT_WIDTH*4)]),   .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[2]), .result(mont_add_result[2]));
CLA33 CLA33_4(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a3[2]}), .B(mul_16_reg[8][(pNTT_WIDTH*8-1):(pNTT_WIDTH*6)]),   .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[3]), .result(mont_add_result[3]));
CLA33 CLA33_5(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a0[3]}), .B(mul_16_reg[8][(pNTT_WIDTH*10-1):(pNTT_WIDTH*8)]),  .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[4]), .result(mont_add_result[4]));
CLA33 CLA33_6(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a1[3]}), .B(mul_16_reg[8][(pNTT_WIDTH*12-1):(pNTT_WIDTH*10)]), .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[5]), .result(mont_add_result[5]));
CLA33 CLA33_7(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a2[3]}), .B(mul_16_reg[8][(pNTT_WIDTH*14-1):(pNTT_WIDTH*12)]), .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[6]), .result(mont_add_result[6]));
CLA33 CLA33_8(.clk(clk), .rst_n(rst_n), .Cin(1'b0), .A({1'b0, mul_16_result_a3[3]}), .B(mul_16_reg[8][(pNTT_WIDTH*16-1):(pNTT_WIDTH*14)]), .in_valid(array_out_valid[0]), .out_valid(mont_add_valid[7]), .result(mont_add_result[7]));
...
CLA17 CLA17_1(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[0]}), .B(Qn), .in_valid(mont_add_valid[0]), .out_valid(mont_n_valid[0]), .result(mont_n_result[0]));
CLA17 CLA17_2(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[1]}), .B(Qn), .in_valid(mont_add_valid[1]), .out_valid(mont_n_valid[1]), .result(mont_n_result[1]));
CLA17 CLA17_3(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[2]}), .B(Qn), .in_valid(mont_add_valid[2]), .out_valid(mont_n_valid[2]), .result(mont_n_result[2]));
CLA17 CLA17_4(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[3]}), .B(Qn), .in_valid(mont_add_valid[3]), .out_valid(mont_n_valid[3]), .result(mont_n_result[3]));
CLA17 CLA17_5(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[4]}), .B(Qn), .in_valid(mont_add_valid[4]), .out_valid(mont_n_valid[4]), .result(mont_n_result[4]));
CLA17 CLA17_6(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[5]}), .B(Qn), .in_valid(mont_add_valid[5]), .out_valid(mont_n_valid[5]), .result(mont_n_result[5]));
CLA17 CLA17_7(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[6]}), .B(Qn), .in_valid(mont_add_valid[6]), .out_valid(mont_n_valid[6]), .result(mont_n_result[6]));
CLA17 CLA17_8(.clk(clk), .rst_n(rst_n), Cin(1'b0), .A({1'b0, mont_R_result[7]}), .B(Qn), .in_valid(mont_add_valid[7]), .out_valid(mont_n_valid[7]), .result(mont_n_result[7]));
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        mont_add_reg[0] <= {mont_R_result[7], mont_R_result[6], mont_R_result[5], mont_R_result[4], mont_R_result[3], mont_R_result[2], mont_R_result[2], mont_R_result[1], mont_R_result[0]};
        mont_add_reg[1] <= mont_add_reg[0];
    end
end

// mod Q
assign result_int1 = (mont_n_result[0][(pNTT_WIDTH)] == 1'b1)? mont_n_result[0][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH-1):0];
assign result_int2 = (mont_n_result[1][(pNTT_WIDTH)] == 1'b1)? mont_n_result[1][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*2-1):(pNTT_WIDTH)];
assign result_int3 = (mont_n_result[2][(pNTT_WIDTH)] == 1'b1)? mont_n_result[2][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*3-1):(pNTT_WIDTH*2)];
assign result_int4 = (mont_n_result[3][(pNTT_WIDTH)] == 1'b1)? mont_n_result[3][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*4-1):(pNTT_WIDTH*3)];
assign result_int5 = (mont_n_result[4][(pNTT_WIDTH)] == 1'b1)? mont_n_result[4][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*5-1):(pNTT_WIDTH*4)];
assign result_int6 = (mont_n_result[5][(pNTT_WIDTH)] == 1'b1)? mont_n_result[5][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*6-1):(pNTT_WIDTH*5)];
assign result_int7 = (mont_n_result[6][(pNTT_WIDTH)] == 1'b1)? mont_n_result[6][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*7-1):(pNTT_WIDTH*6)];
assign result_int8 = (mont_n_result[7][(pNTT_WIDTH)] == 1'b1)? mont_n_result[7][(pNTT_WIDTH-1):0] : mont_add_reg[1][(pNTT_WIDTH*8-1):(pNTT_WIDTH*7)];
